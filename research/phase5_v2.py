...
REPORT     = r"C:\Users\lukas.larsson\Desktop\Privat\Project Reactor Core\Reactor_Core_v2.xlsx"

# ── v2 Portfolio Definition ───────────────────────────────────────────────────
# Changes vs v1: Tesla removed, Lumentum removed, gold capped at 25%, 10Y re-optimized
TICKERS = {
    "xauusd":  "Gold",
    "wmt.us":  "Walmart",
    "lly.us":  "Eli Lilly",
...
LIVE_PRICES_USD = {
    "xauusd": None,    "lly.us": 919.77, "wmt.us": 124.28,
    "avgo.us": 309.51, "cost.us": 996.43, "ccj.us": 108.61,
    "vrt.us": 250.58,  "jnj.us": 244.44,
}
...
ROLES = {
    "xauusd": "Hedge",     "wmt.us":  "Carry",
    "lly.us": "Carry",     "vrt.us":  "Convexity",
    "avgo.us":"Convexity", "ccj.us":  "Cyclical",
    "jnj.us": "Carry",     "cost.us": "Carry",
}
...
c = ws1["A1"]; c.value = "PROJECT REACTOR CORE — v2"
...
c = ws1["A2"]
c.value = ("8 positions | Gold capped 25% | Scarcity thesis: Short Abundance, Long Scarcity | "
           "10Y optimized | 5% min / 40% max per position")
...
r = 4
r = block(ws1, r, "v2 Changes vs v1", [
    ("Removed",      "Tesla (failed audit: +0.010 ΔSharpe when removed, 3/6 regimes only) | "
                     "Lumentum (failed audit: +0.016 ΔSharpe when removed, role overlap)", RED),
    ("Gold cap",     "Reduced from 37.9% to 25% — optimizer was chasing bull run (0.713 momentum correlation). "
                     "Median gold weight across rolling windows: 27.9%. Cost: -0.057 Sharpe.", YEL),
    ("Freed weight", "Redistributed to Walmart (+8.7pp), Eli Lilly (+4.1pp), Vertiv (+4.0pp), Broadcom (+2.9pp)", GRN),
    ("Net result",   "Sharpe 1.851 vs 1.908 original. Framework-adjusted 5Y value $1,392k vs $1,298k (v1). "
                     "Better stress-test resilience: gold -40% costs -26.9pp vs -38.6pp at old weight.", GRN),
])

r = block(ws1, r, f"10Y Performance (2016-2026)", [
...
for row in regime_results:
    ...
    regime_lines.append((
        ...
        f"Portfolio {ret:+.1f}%  |  {vs_str}",
        bg,
    ))
r = block(ws1, r, "Portfolio vs S&P 500 by Regime", regime_lines)

holding_lines = [
    ("Gold (25.0%)",     "Scarcity anchor. Monetary hedge. Capped at 25% vs 37.9% v1 — removes optimizer momentum bias.", YEL),
    ("Walmart (22.7%)",  "Carry compounder. Largest non-gold position. Positive 5/6 regimes. Pricing power moat.", GRN),
    ("Eli Lilly (19.7%)","Carry compounder. Patent-protected GLP-1 monopoly. Constrained manufacturing capacity.", GRN),
    ("Vertiv (9.0%)",    "Convexity. Power infrastructure scarcity. Cannot replicate data center power at this scale.", GRN),
    ("Broadcom (7.9%)",  "Convexity. Custom silicon + networking. Positive 5/6 regimes. Highest single-stock conviction.", GRN),
    ("Cameco (5.7%)",    "Cyclical scarcity. Uranium supply deficit, decade-long mine lead times. Thesis-pure.", GRN),
    ("J&J (5.0%)",       "Carry defensive. Portfolio stabiliser. Near-zero drawdown in most regimes.", GRN),
    ("Costco (5.0%)",    "Carry defensive. Membership moat. Positive 4/6 regimes.", GRN),
]
...
c = ws2["A1"]; c.value = "Final Portfolio — Reactor Core v2 (10Y Optimized, Gold Cap 25%)"
...
for i, (t, w) in enumerate(sorted_w, 1):
    ...
    # precompute all min caps for highlight check
    def _min(tt, ww):
...
c = ws3["A1"]; c.value = "Asset Returns by Regime — Reactor Core v2"
c.font = Font(bold=True, color=WHT, size=13)
c.fill = PatternFill("solid", fgColor=DARK)
c.alignment = Alignment(horizontal="center", vertical="center")
...
c = ws3.cell(row=ri, column=1, value="Portfolio (v2) vs S&P 500 by Regime")
c.font = Font(bold=True, color=WHT); c.fill = PatternFill("solid", fgColor=MID)
...
c = ws4["A1"]; c.value = "Weight Sensitivity — v2 Weights Across Windows + Stress Context"
...
# Compute window weights for display (using hardcoded 10Y, re-optimize 3Y and 5Y)
...
for i, (t, name) in enumerate(TICKERS.items(), 1):
    ...
    # Gold stress: direct impact of -40% gold on portfolio
